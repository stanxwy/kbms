...
async def list_all_departments(session: AsyncSession) -> list[Department]:
    return list((await session.execute(select(Department).order_by(Department.sort_order, Department.id))).scalars())
...
async def exists_department_name(session: AsyncSession, name: str, exclude_id: int | None = None) -> bool:
...
async def count_children(session: AsyncSession, dept_id: int) -> int:
    return (
        await session.execute(select(func.count()).select_from(Department).where(Department.parent_id == dept_id))
    ).scalar_one()


async def count_members(session: AsyncSession, dept_id: int) -> int:
    return (
        await session.execute(select(func.count()).select_from(User).where(User.department_id == dept_id))
    ).scalar_one()
...
async def list_roles_by_ids(session: AsyncSession, role_ids: list[int]) -> list[Role]:
    if not role_ids:
        return []
    return list((await session.execute(select(Role).where(Role.id.in_(role_ids)))).scalars())
...
async def exists_role_code(session: AsyncSession, role_code: str, exclude_id: int | None = None) -> bool:
...
async def list_role_permissions(session: AsyncSession, role_id: int) -> list[RolePermission]:
    return list((await session.execute(select(RolePermission).where(RolePermission.role_id == role_id))).scalars())


async def replace_role_permissions(session: AsyncSession, role_id: int, items: list[tuple[str, str]]) -> None:
